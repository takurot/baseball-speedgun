...
このプロジェクトは、野球の投球速度とバットのスイングスピードを記録し、選手ごとのランキングを表示するためのWebアプリケーションです。ユーザーはアカウントを作成し、自分だけの選手リストと速度記録を管理できます。
...
- **ランキング表示**:
    - 球速とスイングスピードをタブで切り替えて、各種別の最高速度順にランキングを表示します。
    - 同率順位（例: 1位, 2位, 2位, 4位）に正しく対応しています。
    - 1位〜3位にはメダルアイコン（🥇🥈🥉）が表示されます。
- **選手・記録管理 (CRUD)**:
    - フローティングボタンから、選手名、球速またはスイングスピード、日付をモーダルで簡単に追加できます。
    - 同じ選手が同じ日に複数回記録された場合、その日の最高速のみが保存されます。
    - 選手ごとの記録を種別ごとに削除できます。
- **詳細グラフ**:
    - 各選手の日付ごとの球速またはスイングスピード推移を折れ線グラフで視覚的に確認できます。
