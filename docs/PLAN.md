...
### PR-07a: HostingのSPAルーティング（直リンク/リロード）対応
- 目的: `/ranking` などの直リンクで404にならないようにする（React Router対応）
- 背景/課題
  - `https://baseball-speedgun-app.web.app/` は表示できるが、`/ranking` などを直に開くとHosting側で404になる
- 変更内容
  - `firebase.json` のHosting設定に `rewrites`（`**` → `/index.html`）を追加
  - ルーティング追加予定の `/share/:shareId` も含め、SPAの全パスで同様に動作させる
- 完了条件
  - `https://baseball-speedgun-app.web.app/ranking` を直に開いてもアプリが表示される
  - ブラウザのリロードでも同様に表示される
- ステータス: ✅

### PR-07: ランキング共有（リンク）
- 目的: ランキングを他の人に共有しやすくする（閲覧専用）
- 変更内容
  - ランキング画面に「共有」ボタンを追加（リンク作成/コピー/OS共有）
  - 共有リンクの無効化（削除）と、有効期限（例: 7日/30日/無期限）
  - 共有ページ `/share/:shareId` を追加（ログイン不要・閲覧専用UI）
  - Firestoreに共有用コレクション（スナップショット）を追加
  - Firestore rules を拡張（公開読み取り + オーナーのみ作成/削除）
- 完了条件
  - 共有リンクを作成して他者に送れる
...
- PR-07a（Hostingリライト）を先に適用（`/ranking` 直リンク問題の解消 & `/share/:shareId` の前提）
- PR-07はPR-02のランキング確定後に実施（共有はスナップショット方式）

## テスト/確認
- 手動確認
  - 追加/削除/詳細遷移の主要導線
  - 検索/ソート/期間フィルタ
  - 直リンク確認（デプロイ後）: `/ranking` を直接開く・リロードしても表示される
  - 共有リンクの作成/コピー → シークレットで閲覧 → 無効/期限切れの挙動
  - モバイル表示とタップ操作
- 推奨追加テスト（可能なら）
  - ランキング並び/同率順位の表示
  - モーダルバリデーション
  - 期間フィルタのクエリ結果
  - 共有リンクの有効期限（Firestore rules）と公開読み取りの範囲
...
- 成功: `npm test -- --watchAll=false` (PR-06 アクセシビリティ)
- 成功: `npm run build`
